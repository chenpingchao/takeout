...
<body>
    ...
    <!--/*主要部分*/-->
    <?php $image = $menu['image']?"http://www.chpch.top/".$menu['image_dir']."320_".$menu['image'] : '\Public\img\default1.jpg' ?>
    <div class="bodymain">
    ...
    <div class="pl"><!--产品评价-->
        ...
        <?php if(is_array($evaluate)): $k = 0; $__LIST__ = $evaluate;if( count($__LIST__)==0 ) : echo "暂时没有评论" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><div class="pll">
            ...
            <?php $fenshu = ($v['fenshu']/5)*100 ?>
            <div class="pl_2">满意：<div class="bg" style="display:inline-block;"><div style="width:<?php echo ($fenshu); ?>%;"></div></div>
...
<script>
    ...
    function add_cart(){
        ...
        $.post('<?php echo U("Block/addCart",array("uid"=> $menu["id"] ));?>','',function(data){
            if(data.status ==='ok'){
                layer.open({
                    title: [
                        '询问',
