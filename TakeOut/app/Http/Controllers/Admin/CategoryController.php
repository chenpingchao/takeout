    ...
    public function edit($id){
        if (request() -> ajax()){
            ...
            if(request()->has('cate_name')){
            	...
            	$cate_name = Category::where('id',$id) -> value('cate_name');
            }
            ...
            if (Category::where('id',$id)-> update(['pid' => $pid,'update_time'=>time(),'cate_name'=>$cate_name])
            ){
            	if($pid == 0){
                	//更新路径数据
	                if (Category::where('id',$id) -> update(['path' => $id,'update_time'=>time()+1])){
	                    //手动提交
	                    DB::commit();
	                ...
	                $path = $parentPath.','.$id;
	                //更新路径数据
	                if (Category::where('id',$id) -> update(['path' => $path,'update_time'=>time()+1])){
	                    //手动提交
	                    DB::commit();
	                    return response() -> json(['status' => 'ok','msg' => '编辑成功']);
	                }else{
