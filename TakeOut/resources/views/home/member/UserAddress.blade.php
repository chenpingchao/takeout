   ...
   <table class="Myorder">
      ...
      @forelse($mem_msg as $k=>$v)
       <tr style="text-align: center;">
        ...
        <td>{{$v->Postcode}}</td>
        <td>{{$v->mobile}}</td>
        <td><a style="background-color:#84AF9B; border-radius:3px;height:20px;width:40px;color:#F0FFFF;" class="default-active" href="{{route('hm.mem.UserAddress_default',['id'=>$v->id,'active'=>$v->active])}}">{{$v->active==1 ? '默认地址' : '非默认地址'}}</a></td>
        <td>
         <a href="javascript:ovid()" onclick="UserAddress_update('{{$v->name}}','{{$v->id}}','{{$v->site}}','{{$v->Postcode}}','{{$v->mobile}}')" style="background-color:#84AF9B; border-radius:3px;height:20px;width:25px;color:#F0FFFF;" title="编辑">编辑</a> |
         <a class="UserAddress-delete" href="{{route('hm.mem.UserAddress_del',['id'=>$v->id])}}" style="background-color:#84AF9B; border-radius:3px;height:20px;width:25px;color:#F0FFFF;">删除</a>
        </td>
       </tr>
      @empty
       <tr>
 ...
 <script>
  ...
  $('.Submit_b').click(function(){   //submit按钮方式提交
   ...
   if(myvalidate.form()){ //判断前端验证是否通过
     if (id == 0){//判断 添加
      $.post('{{route("hm.mem.UserAddress_add")}}',$('#form'+id).serialize(),function(data){
       //判断是否成功
       if(data.status === 'ok'){
      ...
      $.post('{{route("hm.mem.UserAddress_update")}}',$('#form'+id).serialize(),function(data){
       //判断是否成功
       if(data.status === 'ok'){
        layer.msg(data.msg,{icon:1, time:1000, shade:[0.6]},function(){
         location.href=data.url;
