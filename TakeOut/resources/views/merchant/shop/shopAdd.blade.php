                ...
                <dd>
	                <form id="stepForm"  method="post" enctype="multipart/form-data">
						...
						<select name="sc_id" class="select" >
							@forelse($shop_cate as $v)
								<option value="{{$v->id}}">{{$v->sc_name}}</option>
							@empty
		...
		var s = $("#stepForm").validate({
			rules:{
				...
				shop_mobile:{
					...
					mobile1:true,
					remote:{
						url:"{{route('merchant.shop.shopMobile')}}",
						type:'post',
				...
				audit_mobile:{
					...
					mobile2:true,
					remote:{
						url:"{{route('merchant.shop.auditMobile')}}",
						type:'post',
						data:{_token:'{{csrf_token()}}' }
					}
				},
				...
				Id_number:{
					...
					Id_number:true,
					remote:{
						url:"{{route('merchant.shop.Id_number')}}",
						type:'post',
						data:{_token:'{{csrf_token()}}' }
					}
				}
