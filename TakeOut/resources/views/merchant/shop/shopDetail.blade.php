				...
				<div class="c_detail">
					...
					<div style="background-color:#fff;" class="c_logo">
						<div class="preview-div">
							...
							<img src="{{ $detail -> logo }}" alt="店铺logo" >
						</div>
					</div>
					{{--店铺详细信息--}}
					<div class="c_box companyName">
						<h2 title="剪短名称">{{mb_substr($detail -> shop_name,0,30)}}</h2>
						<div class="clear"></div>

						{{--修改店铺信息的表单--}}
						<form class="clear editDetail dn" id="editDetailForm" method="post" action="{{route('merchant.shop.shopchange')}}">
							{{csrf_field()}}
							<input type="text" placeholder="请输入店铺名" value="{{$detail -> shop_name}}" name="shop_name" id="companyShortName">
							<input type="text" placeholder="店铺电话" value="{{$detail -> shop_mobile}}" name="shop_mobile" id="companyFeatures">
							<input type="text" placeholder="店铺地址" value="{{$detail -> location}}{{$detail -> site}}" name="site" id="companyFeatures">
							<input type="hidden" value="{{$detail -> id}}" id="companyId" name="sid">
						...
						<div class="clear oneword">
							<h1 class="fullname" style="display:inline-block;">店铺评价：{{$detail->grade}}</h1><div class="bg" style="display:inline-block;"><div style="width:{{$detail -> gradebi}}%;"></div></div>
							<h1 class="fullname">平均消费：{{$detail->avg_price}}</h1>
							<h1 title="店铺电话" class="fullname">店铺电话：{{$detail -> shop_mobile}}</h1>
							<h1 title="店铺地址" class="fullname">店铺地址：{{$detail -> site}}</h1>
						...
						<dl class="c_section newIntro dn" id="companyDesFormDiv">
							...
							<dd>
								<form id="companyDesForm" method="post">
									...
									<input type="hidden" value="{{$detail -> id}}" id="sid" name="sid">
                            ...
                            <ul class="hc_list reset">
                                @forelse( $menu as $v)
                                    <li class="width" style="position:relative;">
                                        <a class="active" href="{{route('merchant.shop.menuActive',[ 'uid' => $v->id ,'active'=>$v->active])}} " style="position:absolute;top:5px;right:5px;">
                                            {{$v -> active == 1 ? '上架' : '下架'}}
                                        </a>
                                        <a href="{{route('merchant.shop.menuDetail',[ 'uid' => $v->id ])}}" target="_blank" >
                                            <span style="font-size: 20px">{{$v -> menu_name}}</span>
                                            <div class="comLogo">
                                                <img src="/{{$v -> image_dir .$v -> image ?$v -> image_dir .'190_'.$v -> image : "image/timg.jpg"}}" width="190" height="190" alt="CCIC" />
                                            </div>
                                        </a>
                                        <span>价格：{{$v -> price}}</span>&emsp;&nbsp;
                                        <span>销量：{{$v -> salde_num}}</span><br>
                                        <span>评价数量：{{$v -> eval_num}}</span><br>
                                        <span>关键词：{{$v -> key_words}}</span>
                                    </li>
                                @empty
      	...
      	<!--[if IE 7]> <br /> <![endif]-->
				...
				<dl id="noJobs" class="c_section">
		                    ...
		                    <dd>
		                    	<div class="addnew">
		                        	发布新的菜品，让更多的人知道……<br>
		                            <a href="{{route('merchant.shop.menuAdd',['sid'=> $detail->id])}}">+添加新的菜品</a>
		                        </div>
		                    </dd>
		                </dl>
					...
					<dd>
						<div class="member_wrap">
							...
							<div class="member_info">
								...
								<div class="m_position" style="margin-left: 60px;font-size: 18px;">{{$detail->audit_mobile}}</div>
								<div class="m_intro" style="text-align: center;font-size: 17px;">{{$detail -> e_mail }}</div>
							</div>
							<!-- 编辑创始人 -->
							<div class="member_info newMember dn " id="audit">
								<form class="memberForm" id="audit_form" method="post" action="{{route('/')}}">
									{{csrf_field()}}
									<input type="hidden" value="{{$detail -> id}}" id="companyId" name="sid">
									<input type="text" placeholder="请输入负责人姓名" value="{{$detail -> audit_name}}" name="audit_name">
									<input type="text" placeholder="请输入负责人手机号" value="{{$detail -> mobile}}" name="mobile">
									<input type="text" placeholder="请输入负责人邮箱地址" value="{{$detail -> e_mail}}" name="e_mail">
									<input type="text" placeholder="请输入负责身份证号" value="{{$detail -> Id_number}}" name="Id_number">
			...
			<div id="Category">
				...
				<dl class="c_section c_member">
					...
					<dd style="padding-top:5px;">
						@if(count($menu_cate))
							@foreach($menu_cate as $v)
								@if($v -> active ==1 )
								<a title="菜品分类" class="mc_name" href="{{route('merchant.shop.menuCate',['mc_id'=>$v->id])}}" >
									{{$v-> mc_name}}
								</a>
								@else
								<a title="菜品分类" class="mc_name mc_unactive" href="{{route('merchant.shop.menuCate',['mc_id'=>$v->id])}}" >
									{{$v-> mc_name}}
								</a>
								@endif
							@endforeach
