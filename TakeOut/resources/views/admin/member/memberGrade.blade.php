         ...
         <div class="side_list">
          ...
          <div class="widget-body">
             <ul class="b_P_Sort_list">
                 <li><i class="orange  fa fa-user-secret"></i><a href="?k=0">全部({{$total}})</a></li>
                 @foreach($grades as $k => $v)
                 <li><i class="fa fa-diamond pink "></i> <a href="{{route('admin.member.memberGrade')}}?knum={{$k+1}}">{{$v['grade_name']}}({{$v['member_num']}})</a></li>
                 @endforeach
            </ul>
      </div>
      </div>
         ...
         <span class="Statistic_title Statistic_btn"><a title="隐藏" class="top_close_btn">隐藏</a></span>
          ...
          <!--列表样式-->
           <div class="grading_list">
               ...
               <div class="table_menu_list">
                   ...
                   @forelse($member as $k => $v)
                       <tr>
                           ...
                           <td><a  class='active'  href="{{route('admin.member.active',['id'=>$v -> id,'active'=>$v -> active])}}">{{$v -> active == 1? '激活' : '禁用'}}</a></td>
                       </tr>
                   @empty
                       <tr>
               ...
               <div>共&nbsp;{{$member->total()}}&nbsp;条数据 当前第&nbsp;{{$member->currentPage()}}&nbsp;页</div>
               <div style="float: right">{{$member -> appends(['knum' => $knum]) -> links()}}</div>
           </div>
       </div>
      </div>
    </div>
        ...
        require(
            ...
            function (ec,theme) {
                var myChart = ec.init(document.getElementById('Statistics'),theme);
                 ...
                 $.get( '{{ route('admin.member.getChart') }}' ).done(function (data) {
                     myChart.setOption({
                        title : {
                            text: '用户等级统计',
                            subtext: '实时更新最新等级',
