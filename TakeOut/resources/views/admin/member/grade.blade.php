...
<div class=" page-content clearfix">
        ...
        <form action="">
            <ul class="search_content clearfix">
                <li>
                    ...
                    <input name="detail" type="text" value="{{old('detail')}}" class="text_add" placeholder="输入要搜索的福利内容"  style=" width:250px"/>
                </li>
 ...
 <div id="products_style">
     <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('admin.member.add')}}" title="添加会员等级" class="grade-add btn btn-warning "><i class="icon-plus"></i>添加会员等级</a>
       </span>
     </div>
     <!--产品列表展示-->
    ...
    @forelse($grades as $k => $v)
     <tr>
        ...
        <td width="150px" >
         ...
         <td width="200px" >
             <a href="javascript:detail({{$v -> id}});">{{mb_substr($v -> detail,0,30)}}{{strlen($v -> detail) > 30 ? '...' : ''}}</a>
             <div class="detail" id="{{$v -> id}}">{!! $v -> detail !!}</div>
         </td>
         <td width="100px">
             {{$v -> score}}
         </td>
        <td class="td-manage">
        <a title="编辑"  href="{{route('admin.member.edit',['id' => $v -> id])}}"  class="btn btn-xs btn-info edit" ><i class="icon-edit bigger-120"></i></a>
        <a title="删除" href="{{route('admin.member.delete',['id' => $v -> id])}}"  class="btn btn-xs btn-warning delete" ><i class="icon-trash  bigger-120"></i></a>
       </td>
	  </tr>
        @empty
        <tr>
