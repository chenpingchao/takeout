...
<div class="page-content clearfix">
	...
	<tbody>
		...
		<tr>
          ...
          <td><a class="show" href="{{route('admin.member.show',['id'=>$v->id])}}">{{$v -> username }}</a></td>
          <td>{{$v -> mobile}}</td>
        ...
        <div style="float: right">{{$member -> appends(['username'=>$username,'active'=>$active,'start_time'=>$start_time,'end_time'=>$end_time]) -> links()}}</div>
   </div>
  </div>
 </div>
</div>
...
$('#excel').click(function(){
    location = "{{route('admin.member.excel')}}"
})
</script>