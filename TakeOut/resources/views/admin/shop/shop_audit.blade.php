...
<div class="margin clearfix">
    <div class="Shops_Audit">
        <div class="prompt">当前共有<b>{{$shop_audit_num}}</b>家店铺申请入住</div>
        <!--申请列表-->
        <div class="audit_list">
            <table class="table table-striped table-bordered table-hover" id="sample-table">
                ...
                <tbody>
                @forelse($shop_audit as $k=>$v)
                <tr>
                    <td>{{$v -> id}}</td>
                    <td>{{$v -> shop_name}}</td>
                    <td>{{$v -> sm_name}}</td>
                    <td>{{mb_strcut($v -> detail,0,75)}}</td>
                    <td>{{date('Y-m-d H:i:s',$v -> add_time)}}</td>
                    <td>待审核</td>
                    <td class="td-manage">
                        <a title="店铺详细" href="{{route('bg.shop.detail',['id'=>$v->id])}}" class="btn btn-xs btn-info Refund_detailed">详细</a>
                        <a title="删除" href="{{route('bg.shop.delete',['id'=>$v->id])}}" class="btn shop-delete1 btn-danger" >删除</a>
                    </td>
                </tr>
                @empty
                    <td colspan="7">
