            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <ul class="forminfo" id="goodsInfo" >
                    ...
                    <li>
                        ...
                        <input name="menu_name" type="text" class="dfinput" placeholder="请填写商品名称" value="{{$menu -> menu_name}}" style="width: 680px;"/>
                        <div class="validate-error menu_name" style="background: url('/image/error.png') no-repeat 5px 10px;"></div>
                    </li>
                    <li>
                        <label>菜品关键字<b>*</b></label>
                        <input name="key_words" type="text" class="dfinput" placeholder="请填写商品关键字,多个关键字用逗号隔开" value="{{$menu -> key_words}}" style="width: 680px;"/>
                        <div class="validate-error keywords" style="background: url('/image/error.png') no-repeat 5px 10px;"></div>
                    </li>
                    <li>
                        <label>菜品原价<b>*</b></label>
                        <input name="price" type="text" class="dfinput" placeholder="请填写商城价格" value="{{$menu -> price}}" style="width: 680px;"/>
                        <div class="validate-error price" style="background: url('/image/error.png') no-repeat 5px 10px;"></div>
                    </li>
                    <li>
                        <label>菜品现价<b>*</b></label>
                        <input name="or_price" type="text" class="dfinput" placeholder="请填写市场价格" value="{{$menu -> or_price}}" style="width: 680px;"/>
                    ...
                    <li>
                        ...
                        <div class="preview-div" style="margin-bottom:20px;">
                            ...
                            <img src='/{{$menu -> image_dir}}350_{{$menu -> image}}' width="200" height="200" alt=""/>
                        </div>
                    </li>
                    <li >
                        ...
                        <div style="width: 680px;float:left">
                            @foreach($images as $image)
                                <div class="preview-div" style="float:left;width:200px;height:200px;margin:5px 30px 10px 0;">
                                    <label for="image{{$image->id}}"  class="preview-label" style="top:60px;left:30px;background:rgba(0,0,0,0.6)">重新选择</label>
                                    <input type="file" id="image{{$image->id}}" name="image[{{$image->id}}]" class="preview-input"  onchange="preview(this,200,200)">
                                    <img src='/{{$image->image_dir}}350_{{$image->image}}' width="200" height="200" alt=""/>
                                </div>
                            @endforeach
                    ...
                    <li>
                        ...
                        <textarea name="detail" id="content" style="width: 680px"  rows="30">{{$menu -> detail}}</textarea>
                    </li>
                    <li>
                        <button type="submit" style="width:137px;height:35px; background:url(/admin/images/btnbg.png) no-repeat; font-size:14px;font-weight:bold;color:#fff; cursor:pointer;">确认编辑</button>
                    </li>
                </ul>
            </form>
        </div>
...
<script type="text/javascript">
    ...
    //获取所有的顶级分类作为一级分类菜单内容
    getSubCate(firstCate,0,{{$cates[0]}});
    getSubCate(secondCate,{{$cates[0]}},{{$cates[1]}});
    getSubCate(thirdCate,{{$cates[1]}},{{$cates[2]}});
