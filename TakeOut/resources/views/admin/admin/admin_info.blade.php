        ...
        <div class="admin_modify_style" id="Personal">
            ...
            <div class="xinxi">
                ...
                <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">用户名： </label>
                    <div class="col-sm-9">
                        <input type="text" name="username"  value="{{$admin->username}}" class="col-xs-7 text_info" disabled="disabled">&nbsp;&nbsp;&nbsp;
                ...
                <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">移动电话： </label>
                    <div class="col-sm-9">
                        <input type="text" name="moble" id="" value="{{$admin->moble}}" class="col-xs-7 text_info" disabled="disabled">
                    </div>
                </div>

                <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">电子邮箱： </label>
                    <div class="col-sm-9">
                        <input type="text" name="email" value="{{$admin->email}}" class="col-xs-7 text_info" disabled="disabled">
                    </div>
                </div>
                ...
                <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">注册时间： </label>
                    <div class="col-sm-9" > <span>{{$admin->add_time?date('Y-m-d H:i:s',$admin->add_time):'开天之物'}}</span></div>
                </div>
        ...
        <div class="recording_style">
            ...
            <div class="recording_list">
                ...
                <table class="table table-border table-bordered table-bg table-hover table-sort" id="sample-table">
                    ...
                    @forelse($admin_logs as $k=>$v)
                    <tbody>
                    <tr>
                        <td>{{$admin_logs->firstItem()+$k}}</td>
                        <td>{{$v->active==1?'激活':'禁用'}}</td>
                        <td>
                            {{$v->active==1?'登陆成功':'登录失败'}}
                        </td>
                        <td>{{$v->ad_name}}</td>
                        <td>{{$v->login_ip}}</td>
                        <td>{{$v->login_time?date('Y-m-d H:i:s',$v->login_time):'开天之物'}}</td>
                    </tr>
                    </tbody>
                    @empty
                        <tr>
                    ...
                    <div>共{{$admin_logs->total()}}条记录,当前页为第{{$admin_logs->currentPage()}}页</div>
                    <div align="right">
                        {{$admin_logs->links()}}
                    </div>
                </form>
...
<script type="text/javascript">
    $('form').submit(function(){
        $.ajax({
            url:'{{route('bg.admin.up_pwd',['id'=>$admin['id']])}}',
            type:'post',
            data:$(this).serialize(),
            dataType:'json',
...
<script type="text/javascript">
    $('#admin_update').click(function () {
        $.post('{{route("bg.admin.update",['id'=>$admin['id']])}}', $('#form1').serialize(), function (data) {
            if(data.status === 'ok'){
                layer.msg(data.msg,{icon:6,shade:[0.6]},function(){
                    //重定向窗口
                    location = data.url;
