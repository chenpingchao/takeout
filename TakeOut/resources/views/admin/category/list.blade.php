 ...
 <div id="products_style">
    <div class="search_style">
        <form action="">
          <ul class="search_content clearfix">
           <li>
               ...
               <input name="cate_name" type="text" value="{{old('cate_name')}}" class="text_add" placeholder="输入分类名称"  style=" width:250px"/>
           </li>
          <li>
     ...
     <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('admin.category.add')}}" title="添加分类" class="cate-add btn btn-warning "><i class="icon-plus"></i>添加顶级分类</a>
       </span>
     </div>
     <!--产品列表展示-->
    ...
    @forelse($category as $k => $v)
     <tr path="{{$v -> path}}">
        <td width="80px" >{{$category -> firstItem() + $k}}</td>
        <td width="250px" style="text-align: left">{{$v -> cate_name}}
            @if($v -> child_num > 0)
                <span class="extend" url="{{route('admin.category.childList',['pid'=>$v->id]) }}">+</span>
            @endif
        </td>
        <td width="250px">{{$v -> parent_name or '顶级分类'}}</td>
        <td class="td-status">
            <span url="{{route('admin.category.active',['path'=> $v -> path,'active'=> $v -> active]) }}" class="label {{$v -> active == 1 ? 'label-success' : 'label-defaunt'}}  radius{{$v->id}} active">
                {{$v -> active == 1 ? '激活' : '停用' }}
            </span>
        </td>
        <td class="td-manage">
    ...
    $.get($(this).attr('url'),'',function (data) {
        if (data.status === 'ok'){
            ...
            $('tr[path^="'+ data.path+',"]').each(function () {
                //拼接新的url地址
                var url = "{{url('bg/category/active')}}"+'/'+$(this).attr('path')+'/'+data.active;
                //更改属性
