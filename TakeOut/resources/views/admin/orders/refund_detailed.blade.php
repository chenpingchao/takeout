...
<div class="margin clearfix">
 <div class="Refund_detailed">
    <div class="Numbering">退款单编号:<b>{{$data['msg'][0] -> orders_num }}</b></div>
    <div class="detailed_style">
     ...
     <div class="Info_style clearfix">
        <div class="col-xs-3">  
         ...
         <div class="o_content">{{$data['msg'][0] -> username }}</div>
        </div>
        <div class="col-xs-3">  
         <label class="label_name" for="form-field-2"> 退款人电话： </label>
         <div class="o_content">{{$data['msg'][0] -> moble }}</div>
        </div>
         <div class="col-xs-3">  
         <label class="label_name" for="form-field-2"> 退款数量：</label>
         <div class="o_content">{{$data['orders_menu_num']}}</div>
        </div>
        <div class="col-xs-3">  
         <label class="label_name" for="form-field-2"> 退款金额：</label>
         <div class="o_content">{{$data['msg'][0] -> orders_price }}元</div>
        </div>
        <div class="col-xs-3">
         <label class="label_name" for="form-field-2"> 退款日期：</label>
         <div class="o_content">{{date('Y-m-d h:i:s',$data['msg'][0] -> add_time ) }}</div>
        </div>
        <div class="col-xs-3">  
         <label class="label_name" for="form-field-2"> 状态：</label>
         <div class="o_content">
             @if( $data['msg'][0] -> active == 6 )
                 <span>待退货</span>
             @else
                 <span>已退货</span>
             @endif
    ...
    <div class="Info_style clearfix">
        @foreach($data['menu'] as $k =>$v )
            <div class="product_info clearfix">
                <a href="{{$v -> id}}" class="img_link">
                    <img src="/admin/products/p_3.jpg">
                </a>
                <span>
                    <a href="{{ $v -> id}}" class="name_link" style="font-size:25px;color:#CC48B2;">{{ $v -> menu_name}}</a><br>
                    <p>数量：{{ $v -> num }}件</p>
