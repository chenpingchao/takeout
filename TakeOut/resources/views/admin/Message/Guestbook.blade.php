    ...
    <div class="margin clearfix">
        <div class="Guestbook_style">
                ...
                <form action="" method="post">
                    ...
                    <ul class="search_content clearfix">
                        ...
                        <li><label class="l_f">添加时间</label>
                            <input name="add_time" class="inline laydate-icon" value="{{date(" Y-m-d ",$add_time ? $add_time : 1352550280) }}" id="start" style=" margin-left:10px;">
                        </li>
                        <select name="active" id="">
                            <option value="" {{$active != 1 && $active == !2 ? 'selected' : ''}}>全部</option>
                            <option value="1" {{$active == 1 ? 'selected' : ''}}>已回复</option>
                            <option value="2" {{$active == 2 ? 'selected' : ''}}>未回复</option>
                        </select>
                        <li style="width:90px;">
            ...
            <div class="border clearfix">
                <span class="l_f">
                    <a href="{{route('bg.mess.GDeletes')}}" class="Guest-deletes btn btn-danger"><i class="fa fa-trash"></i>&nbsp;批量删除</a>
                </span>
                <span class="r_f">共：<b>{{$Guestbook->total()}}</b>条</span>
            </div>
            <!--留言列表-->
            <div class="Guestbook_list">
                ...
                <table class="table table-striped table-bordered table-hover" id="sample-table">
                    ...
                    <tr>
                        ...
                        <td>{{$Guestbook->firstItem()+$k}}</td>
                        <td>
                            <a href="{{route('bg.mess.MemberShow',['id'=>$v->id])}}" style="cursor:pointer;color:dodgerblue;"  class="text-primary">{{$v->username}}</a>
                        </td>
                        <td class="text-l">
                            <a href="javascript:;" onclick="Guestbook_iew('{{$v->id}}','{{$v->username}}','{{$v->content}}','{{$v->mid}}')" style="overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:3;">{{$v->content}}</a>
                        </td>
{{--                        {{route('bg.mess.GuestOne',['id'=>$v->id])}}--}}
                        <td>{{date('Y-m-d H:i:s',$v->add_time)}}</td>
                        <td class="td-status">
                            <span class="label label-success radius">
                                <a style="color:beige;" href="{{route('bg.mess.GuestActive',['id'=>$v->id,'active'=>$v->active])}}" class="admin-active tablelink">
                                {{$v->active==1 ? '已回复' : '未回复'}}</a>
                            </span>
                        </td>
                        <td class="td-manage">
                            <a href="javascript:;"  onclick="Guestbook_iew('{{$v->id}}','{{$v->username}}','{{$v->content}}','{{$v->mid}}')" title="回复" class="btn btn-xs btn-pink" ><i class="fa fa-envelope-o  bigger-120"></i></a>
                            <a href="{{route('bg.mess.GDelete',['id'=>$v->id])}}"  onclick="member_del(this,'1')" title="删除" class="Guest-delete btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
                        </td>
                    </tr>
                    </tbody>
                    @empty
                ...
                <div>当前页共{{$Guestbook->count()}}条记录,为第{{$Guestbook->currentPage()}}页</div>
                <div align="right">
                    {{$Guestbook->appends([
                      'username'=>$username,
                      'add_time'=>$add_time,
