                ...
                <dd>
	                <form id="stepForm"  method="post" enctype="multipart/form-data">
						...
						<select name="sc_id" class="select" >
							<?php $__empty_1 = true; $__currentLoopData = $shop_cate; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
								<option value="<?php echo e($v->id); ?>"><?php echo e($v->sc_name); ?></option>
							<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
								<option value="1">其他</option>
							<?php endif; ?>
						</select>
						<div class="image sc_id"></div>
	                    ...
	                    <h3>所在城市</h3>
							<select name="province" id="s1" class="xuanze" ></select>
							<select name="city" id="s2" class="xuanze" ></select>
							<select name="town" id="s3" class="xuanze" ></select>
...
<?php $__env->startSection('script'); ?>
	<script src="/js/geo.js" charset="UTF-8" type="text/javascript"></script>
	<script>
		//省市县三级联动
		setup();
		preselect('河南省');
		...
		var s = $("#stepForm").validate({
			rules:{
				...
				Id_number:{
					...
					Id_number:true,
					remote:{
						url:"<?php echo e(route('merchant.shop.Id_number')); ?>",
						type:'post',
						data:{_token:'<?php echo e(csrf_token()); ?>' }
					}
				}
		...
		$('#stepForm').submit(function(){
            ...
            if(s.form()){
				...
				me.ajaxSubmit({
					...
					error:function (xhr) {
						var errors = JSON.parse(xhr).errors;
						//清空前台表单验证
						$('div.image').html('');
						//将错误信息写入表单
						for(var i in errors){
